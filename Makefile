...
ifdef CI
    ...
    export VENV_PATH := /tmp/darca-repository-venv
    export POETRY_HOME := /tmp/poetry-repository-cache
    export POETRY_CONFIG_DIR := /tmp/poetry-repository-config
    export POETRY_CACHE_DIR := $(POETRY_HOME)
    export POETRY_VIRTUALENVS_PATH := $(VENV_PATH)
    export PYTHONPYCACHEPREFIX := /tmp/repository-pycache
    ...
    export PRE_COMMIT_HOME := /tmp/precommit-repository-cache
endif
